...
Target: `ry-install.fish` **v7.91.0** (attached). Source of truth: script > README > CHANGELOG.
...
**Counts (from `_ir_validate_counts`, all 19 hard-asserted — UNCHANGED from v7.88.3):** KERNEL_PARAMS **17** · MKINITCPIO_HOOKS 11 · MKINITCPIO_MODULES 1 · LOGIND_IGNORE_KEYS 8 · ENV_VARS 11 · SYSCTL_VALUES 9 · PKGS_ADD 17 · PKGS_DEL 9 · MASK 10 · EXPECTED_VULKAN_PKGS 2 · EXPECTED_SERVICES 5 · _RY_PKG_MANAGED_SERVICES 1 · _RY_POST_HOOKS **17** · _RY_BOOT_CRITICAL_DSTS 4 · _RY_PHASE_NAMES 6 · _RY_BACKUP_TARGETS 2 · _RY_TMPDIR_GLOBS 6 · SYSTEM_DESTINATIONS 15 · USER_DESTINATIONS **2**. Managed files = **17** (15 system + 2 user; `_RY_MANAGED_FILE_COUNT=17`). MangoHud.conf = **19 active directives + 1 commented (`# cpu_temp`)** — the v7.91.0 body **RE-COMMENTED `cpu_temp` and ADDED `cpu_power`** (net wash: active count stays 19, but composition inverted vs v7.88.3), and **reordered `gpu_temp` before `gpu_core_clock`**. The v7.88.3 "19 active, 0 commented, `cpu_temp` LIVE, no `cpu_power`" body is superseded — see §12/§B9. NTSYNC autoload confs = **0** (de-managed v7.76.1; ntsync is assert-only).
...
## What changed since v7.88.3 (this re-audit's delta — READ FIRST)

The prior revision targeted v7.88.3. The script is now **v7.91.0** (three releases ahead: 7.89.0, 7.90.0, 7.91.0). Function count dropped **294 → 288**, script **5080 → 4951 lines**. The following are NEW, INVERTED, or REMOVED between v7.88.3 and v7.91.0 and OVERRIDE the corresponding v7.88.3 conclusions. Every item below is re-derived directly from `ry-install.fish` v7.91.0.

1. **MangoHud `cpu_temp` RE-COMMENTED + `cpu_power` ADDED + GPU order swapped (v7.91.0) — THE HEADLINE INVERSION.** The v7.88.3 headline ("`cpu_temp` is now LIVE/uncommented; `cpu_power` NOT present; 0 commented") is now **wrong in every particular**. The shipped body (generator `_content_HOME_.config_MangoHud_MangoHud.conf`, lines 992–1015) now carries `# cpu_temp` (commented, line 1007) and a live `cpu_power` (line 1009, after `cpu_mhz`), and lists `gpu_temp` (1003) **before** `gpu_core_clock` (1004) — the reverse of v7.88.3. Active-directive count is *coincidentally still 19* (a −`cpu_temp` / +`cpu_power` wash), so the header count did not move, but the composition and ordering did. This RESTORES the pre-v7.88.3 (v7.77.1) `cpu_temp`-commented posture and additionally adds `cpu_power`. Consequence for the analysis: the entire "is `cpu_temp` populating from k10temp / MangoHud #1825 sensor-key / #1794 cpu_power-zeroing" investigation now inverts — `cpu_power` is LIVE and `cpu_temp` is OFF, so **MangoHud #1794 (enabling `cpu_temp` can zero `cpu_power` on Zen 5) is now avoided by design**, not merely moot. Re-verify `cpu_power` populates from amdgpu/k10temp on Zen 5 as shipped; the `cpu_temp` sensor-key question is now dormant (re-enabling it is a documented user edit, not a shipped directive). See §12/§B9/§F.
2. **Six `--verify` subs REMOVED (v7.90.0).** `_vre_tcp`, `_vre_zram`, `_vss_ntsync_modules`, `_vrkm_iommu`, `_vrk_clocksource`, and the `_RY_DMESG_TSC` cache are **gone** (confirmed absent, 0 occurrences each). The Vulkan-package check was **folded into `_vsp_required`** (carrying the pacman-db-lock guard), so the standalone `_vrs_vulkan` runtime-session leaf is **also gone** (0 occurrences). This substantially rewrites §C — the static/runtime orchestrator leaf inventories below are corrected accordingly. NOTE: `_vre_ntsync` and the `_ntsync_state` machine **survive** (only the *static* `_vss_ntsync_modules` was dropped); `_vrkm_amdgpu` and `_vrkm_blacklist` survive under `_vrk_module_state` (only `_vrkm_iommu` was removed from it).
3. **IOMMU + TSC live-effect asserts RETIRED, directive-level asserts KEPT (v7.90.0).** With `_vrkm_iommu` and `_vrk_clocksource` gone, the installer no longer hard-fails on iommu_groups-present-under-`amd_iommu=off`, and no longer HPET-fails / correlates TSC-demotion from dmesg. **BUT `amd_iommu=off` and `tsc=reliable` are STILL asserted** at two layers: the config layer (`/etc/kernel/cmdline` token set, `_vsb_cmdline`) and the live-cmdline layer (`_vrk_cmdline`'s generic KERNEL_PARAMS loop asserts every token present in `/proc/cmdline`). Only the *sysfs-effect* correlations were dropped — the directives themselves remain byte- and runtime-asserted. Every "`_vrkm_iommu` hard-fails if groups present" and "`_vrk_clocksource` HPET→fail" statement in the v7.88.3 body is now retired; the README AMD-Vi note was synced to match (v7.90.0).
4. **Root guard now defers to argparse (v7.89.0).** Invalid args exit `2` with the *same* usage message whether or not the run is rooted (the root guard no longer pre-empts arg validation). Minor; touches §K/CLI-contract framing only, no tuning value.
5. **Stale orchestrator description strings (NEW cosmetic finding, Low/Low).** Three `--verify` orchestrators still carry v7.88.3-era `--description` text naming subs they no longer call: `_verify_static_system` says "Verify ntsync, resolved, …" (no longer calls `_vss_ntsync_modules`); `_verify_runtime_kparams` says "…, clocksource" (no longer calls `_vrk_clocksource`); `_verify_runtime_env` says "…, TCP, ZRAM, …" (no longer calls `_vre_tcp`/`_vre_zram`). These are doc-string drift only — the call bodies are correct — but they are the kind of stale-comment finding this audit surfaces (FIX-doc, Low/Low): trim the description strings to match the current call set.

**Everything else the v7.88.3 body asserted about tuning VALUES is unchanged** — KERNEL_PARAMS (17), ENV_VARS (11), SYSCTL_VALUES (9), PKGS_ADD/DEL (17/9), MASK (10), the nftables IPv4-only ruleset + inbound-ping accept, the udev matchers (`cpu[0-9]*` EPP, `card[0-9]*`+`drm_minor` GPU), `amd_iommu=off`, `ipv6.disable=1`, `dynamic_epp=disabled`, remote-play port set incl. 27037, KERNEL_MIN 6.19, all verified byte-for-byte against v7.91.0. The MES-0x86 label conflict (below) is unchanged and remains the top factual reconciliation.

## What changed since v7.77.1 (prior re-audit focus — retained for lineage)

The v7.77.1→v7.88.3 deltas below are retained because the current script still reflects them (they were not re-reverted in 7.89–7.91), EXCEPT item 4 (MangoHud `cpu_temp`), which **v7.91.0 re-inverted** — see "What changed since v7.88.3" item 1 above; the v7.88.3 "cpu_temp is LIVE" claim in item 4 is now superseded. Between v7.77.1 and v7.88.3 the script moved substantially. The following are NEW, INVERTED, or REMOVED and need fresh evidence — do NOT carry forward the old (v7.77.1) conclusions on these:
...
3. **`amd_iommu=off` unchanged** (still v7.77.0's inversion; `iommu=pt` remains gone). AMD-Vi stays fully disabled. **NOTE (v7.90.0):** the `_vrkm_iommu` sysfs-effect assert (off-derivation → expect 0 iommu_groups, hard-fail if present) was **REMOVED** — see "What changed since v7.88.3" item 3; the `amd_iommu=off` *directive* is still asserted at the cmdline layer, but the 0-groups live check is gone. Re-evaluate on its own merits exactly as before (latency vs DMA-isolation cost); the README VFIO/SR-IOV escape hatch (`amd_iommu=on iommu=pt`) is still documented. ROCm on gfx1151 is unaffected (`IOMMU Support: None`).
4. **~~MangoHud `cpu_temp` is now LIVE (UNCOMMENTED)~~ — SUPERSEDED by v7.91.0.** This v7.88.3 finding is **no longer true.** v7.91.0 re-commented `cpu_temp` and added a live `cpu_power` (see "What changed since v7.88.3" item 1). The shipped v7.91.0 body carries `# cpu_temp` (commented) and live `cpu_power`, so the current posture is the *inverse* of what this item claimed. Do NOT verify `cpu_temp` as a live directive; verify `cpu_power` instead (§12/§B9). MangoHud #1794 (`cpu_temp` zeroing `cpu_power`) is now avoided by design — `cpu_temp` is off, `cpu_power` is on. The k10temp/#1825 sensor-key question is dormant.
...
> **Verification note (2026-07-04).** This revision re-derives every value directly from `ry-install.fish` **v7.91.0** (extracted array-by-array, 288 fns). Where the script and any prior prompt disagree, the script wins and the disagreement is flagged (headline: the **MES 0x86 label** conflict above, and the **inbound-IPv4-ping / IPv6-disabled** network inversion). The v7.88.3→v7.91.0 deltas ("What changed since v7.88.3" above) are folded throughout: the **MangoHud `cpu_temp`-re-commented + `cpu_power`-added** inversion (v7.91.0), the **six removed `--verify` subs** (`_vre_tcp`/`_vre_zram`/`_vss_ntsync_modules`/`_vrkm_iommu`/`_vrk_clocksource`/`_vrs_vulkan`, v7.90.0), and the **retired IOMMU/TSC effect-correlations** (directives still asserted). Live-upstream anchors that the script did NOT contradict (ntsync CONFIG_NTSYNC=y, amd_iommu=off not breaking ROCm, governor/EPP semantics, §13 candidate calls) are retained. **Correction to prior §C:** the `_vmh_*` leaves are **`_ry_validate_mkinitcpio_hooks` sub-functions (mkinitcpio HOOKS existence/ordering)**, NOT MangoHud-verify leaves — the prior body misattributed them; MangoHud static verify is the inline `_chk_grep fps` + the `_grep_mangohud_entry` format-validator. §C now maps the actual v7.91.0 leaf-to-orchestrator set (`_vsb_*`, `_vsp_*` incl. the folded Vulkan check, `_vsc_check_one`, `_vpd_dir_perm_check`). The full 14-code exit contract + `_run` 1 h timeout are documented in §K. Internal-state globals (progress-bar, matrix counters, boot-enum scratch) are deliberately out of scope — implementation scaffolding, not tuning targets.
...
cat /sys/.../clocksource0/current_clocksource               # tsc — INFORMATIONAL ONLY as of v7.90.0 (script no longer HPET-fails; _vrk_clocksource removed)
...
find /sys/kernel/iommu_groups -mindepth 1 -maxdepth 1 -type d | wc -l   # 0 — INFORMATIONAL ONLY as of v7.90.0 (script no longer hard-fails on groups-present; _vrkm_iommu removed)
...
swapon --show; zramctl                                      # zram active (advisory; profile does not manage zram — AND no longer asserts it: _vre_zram removed v7.90.0)
...
printenv MANGOHUD                                          # 1
grep -c '^cpu_temp' ~/.config/MangoHud/MangoHud.conf       # 0 (cpu_temp is RE-COMMENTED as of v7.91.0)
grep -c '^cpu_power' ~/.config/MangoHud/MangoHud.conf      # 1 (cpu_power ADDED v7.91.0)
grep -c '^# cpu_temp' ~/.config/MangoHud/MangoHud.conf     # 1 (the single commented directive)
```

Hard `--verify` asserts (mismatch → exit 1/3): every `KERNEL_PARAMS` token present in `/proc/cmdline` + `rw` (generic loop in `_vrk_cmdline`, so `amd_iommu=off` AND `ipv6.disable=1` AND `tsc=reliable` are all auto-asserted **at the cmdline layer**); scaling_driver=amd-pstate-epp, scaling_governor=powersave, EPP=balance_performance, **amd_pstate `dynamic_epp=disabled`**, amd_pstate status/prefcore, boost; GPU `power_dpm_force_performance_level=auto`; usbcore.autosuspend=-1, nvme_core.default_ps_max_latency_us=0, zswap.enabled∈{N,0}, nmi_watchdog=0, NVMe sched `[none]`; regdom (`/etc/iw-regdomain`); **nftables IPv4 ping accept (`echo-request`) present (`_vss_nft`; regression guard — the ruleset MUST keep inbound ping enabled)**; mt7925e `disable_aspm=1`; NM system-connections 0600 root:root (`_vrs_nm_perms`); PKGS_ADD present + **Vulkan pkgs (vulkan-radeon + lib32-vulkan-radeon) — folded into `_vsp_required` v7.90.0**. Warn-level: GPU DPM not-`auto`, live nftables ping cross-check (`_vrsv_nft_assert_ping`, warn-only), ntsync runtime device state (`_vre_ntsync` — survives), iwd runtime state (opt-in). **REMOVED asserts (v7.90.0 — DO NOT verify): the IOMMU sysfs-effect hard-fail (`_vrkm_iommu`, 0-iommu_groups check), the clocksource=tsc / HPET→fail + TSC-demotion correlation (`_vrk_clocksource`), the ZRAM state check (`_vre_zram`), and the `tcp_bbr` runtime check (`_vre_tcp`).** The `amd_iommu=off` and `tsc=reliable` DIRECTIVES are still asserted (cmdline token loop + `_vsb_cmdline` static), but their *live sysfs effects* are no longer checked. No THP, KSM, `ttm.*`, drirc, `radv_enable_unified_heap_on_apu`, `iommu=pt`, ICMPv6/NDP, baloo, `_kb_*`, iommu-groups, or clocksource-sysfs assert exists — do not verify them. NOTE: the nftables assert is the **ping accept**, NOT an NDP rule; the firmware-version advisory no longer exists.
...
- **`amd_iommu=off` (unchanged):** AMD-Vi fully off; no `iommu=pt`, no `amd_iommu=on`. **`amd_iommu=off` does NOT break ROCm** — `rocminfo` reports `IOMMU Support: None` on gfx1151 and the Strix Halo LLM-toolbox community *recommends* `amd_iommu=off` for the 128 GB unified pool (no SVM/large-allocation penalty). Not a compute regression. Re-evaluate the remaining parts: (a) the gaming/compute latency win of disabling vs `iommu=pt` translation-bypass on Zen 5 (likely marginal); (b) the DMA-isolation loss (§10); (c) the README opt-back-in (`amd_iommu=on iommu=pt`, re-run — **NOTE:** as of v7.90.0 the `_vrkm_iommu` "groups populated" verify assert was removed, so re-enabling no longer live-verifies group creation; the directive is still cmdline-asserted). Net: accepted single-user-desktop trade-off, not a free latency win, not a compute regression.
...
**MangoHud.conf (19 active directives + 1 commented `# cpu_temp`, 0600, fps/frametime first) — v7.91.0 body:** horizontal, legacy_layout=0, position=top-left, toggle_hud=Shift_R+F12, fps, frametime, frame_timing, gpu_stats, **`gpu_temp` (moved BEFORE gpu_core_clock, v7.91.0)**, gpu_core_clock, gpu_power, cpu_stats, **`# cpu_temp` (RE-COMMENTED, v7.91.0)**, cpu_mhz, **`cpu_power` (ADDED, v7.91.0, after cpu_mhz)**, vram, ram, font_size=20, text_outline, background_alpha=0.4. Still fully absent: fps_metrics. Enabled via MANGOHUD=1. bluetooth main.conf: FastConnectable=true, AutoEnable=true, ReconnectAttempts=3 (no explicit ReconnectIntervals — BlueZ default backoff). **baloofilerc DE-MANAGED (v7.77.0) — no longer written; USER_DESTINATIONS = 2.**

- **MangoHud `cpu_temp` is RE-COMMENTED and `cpu_power` is now SHIPPED LIVE (v7.91.0 — inverted again vs v7.88.3):** the profile now ships `# cpu_temp` (commented) and a live `cpu_power` bareword (after `cpu_mhz`). This is the *reverse* of the v7.88.3 body. Consequences: (a) **MangoHud #1794 is now AVOIDED BY DESIGN** — the #1794 hazard is that enabling `cpu_temp` can zero `cpu_power` on Zen 5; with `cpu_temp` OFF and `cpu_power` ON, the interaction cannot trigger. This is arguably *why* the swap was made (prioritizing the power readout, which is the more useful metric on a 140 W-capped APU where thermals rarely bind before power does). (b) The `cpu_temp` sensor-key / #1825 / `cpu_temp_sensor=k10temp` question is now **dormant** — re-enabling `cpu_temp` is a documented user edit, not a shipped directive; if a user uncomments it they may still need the sensor key and would re-trip #1794, so a README note to that effect would be worth adding (ADD-doc, Low/Low). (c) **The real live-verification target is now `cpu_power`:** confirm it populates from amdgpu/k10temp power sensors on Zen 5 (Strix Halo) under Wayland as shipped — MangoHud reads Zen `cpu_power` from the RAPL/`power1_average` hwmon; confirm the installed MangoHud + 6.19 kernel expose it. If `cpu_power` reads blank/zero, that is the FIX to investigate (not `cpu_temp`); if it populates, KEEP.
- Confirm all 19 active directives are valid for current MangoHud (do not invent), including the newly-live `cpu_power`. With both `gpu_power` and `cpu_power` present, confirm each populates from its respective sensor path on gfx1151/Zen 5 under Wayland. Confirm the `gpu_temp`-before-`gpu_core_clock` reorder (v7.91.0) is purely cosmetic (display order) with no semantic effect. Confirm `frame_timing` (graph) vs `frametime` (numeric) are both current and not redundant. Confirm `toggle_hud=Shift_R+F12` is a valid current bind.
...
- **Reinstatement rule.** Items deliberately removed/disabled (do not recommend reinstating unless current upstream directly contradicts the rationale — then flag, not FIX): `amdgpu.ppfeaturemask` (re-evaluate per §13a as an undervolt/OC opt-in, not a silent default), `--country` flag, TTM/GTT cap, RADV drirc, MangoHud `fps_metrics`, `vm.page-cluster`/`vm.vfs_cache_pressure` (vendor-provided), ntsync `modules-load.d` autoload conf (now assert-only), baloofilerc (de-managed v7.77.0), the `_kb_*` benign-INFO subs + `_ry_check_umip_disabled` (removed v7.77.0), ICMPv6/NDP nftables rules (dropped v7.87.x — do NOT re-add without restoring IPv6). Live config to evaluate as KEEP-or-FIX-to-remove (not protected): PROTON_FSR4_RDNA3_UPGRADE, MangoHud gpu_power/text_outline/toggle_hud/**cpu_power (now live, v7.91.0)**, `ipv6.disable=1`, inbound-ping accept. **`cpu_temp` is now COMMENTED (v7.91.0)** — treat re-enabling it as a user opt-in (would re-trip MangoHud #1794), not a shipped directive to evaluate.
...
### B9. `~/.config/MangoHud/MangoHud.conf` (exact — v7.91.0 body: 19 active + 1 commented + 1 file-header, ordered)
```
...
```
- **v7.91.0 diff vs the v7.88.3 body (three changes):** (1) `gpu_temp` moved BEFORE `gpu_core_clock`; (2) `cpu_temp` RE-COMMENTED to `# cpu_temp`; (3) `cpu_power` ADDED after `cpu_mhz`. Active-directive count is unchanged (19) because −`cpu_temp`/+`cpu_power` is a wash. The file-header comment (`# ry-install: …`) is present in the generator (line 994) but is not a directive; excluding it, the body is **19 active + 1 commented directive**.
- **`cpu_temp` is COMMENTED, `cpu_power` is LIVE** (the inverse of v7.88.3). Verify `cpu_power` populates from the Zen 5 RAPL/`power1_average` hwmon on the installed MangoHud + 6.19 kernel under Wayland. The `cpu_temp` sensor-key / #1825 question is dormant (it is not shipped active); MangoHud #1794 (cpu_temp zeroing cpu_power) is avoided by design. If `cpu_power` reads blank → FIX-to-investigate; if it populates → KEEP. Confirm `frame_timing` (graph) vs `frametime` (numeric) both valid; confirm `toggle_hud=Shift_R+F12` syntax current; confirm the `gpu_temp`/`gpu_core_clock` reorder is display-only.

## C. Full verify subsystem (`--verify`) — orchestrator families

The top VERIFY block is the user-facing command set; the script's actual `--verify` runs a set of orchestrator functions across sub-families. Recommendations that change a value MUST state which sub asserts it (and whether hard-fail or warn). The architecture below is re-derived from **v7.91.0** (288 fns). **Six leaves removed in v7.90.0** are marked ⊘; **the `_vmh_*` misattribution from the prior body is corrected** (they belong to mkinitcpio-hook validation, not MangoHud verify).

**Static (on-disk content) — 5 orchestrators + syntax + checksum:**
- `_verify_static_boot` → leaves `_vsb_loader` (loader.conf key=values) · `_vsb_sdboot` (sdboot-manage.conf) · `_vsb_cmdline` (`/etc/kernel/cmdline` token set — this is where `amd_iommu=off`/`ipv6.disable=1`/`tsc=reliable` are byte-asserted) · `_vsb_mkinitcpio` (HOOKS/MODULES/COMPRESSION incl. `LINUX_FALLBACK_OPTIONS="quiet"`) · `_vsb_entries` (generated BLS entries). All leaves hard-fail static.
- `_verify_static_system` → subs `_vss_logind` · `_vss_nmdispatch` · `_vss_nm` · `_vss_sysctl` (key=value) · `_vss_regdom` · `_vss_bluetooth` · resolved (inline `_chk_grep` loop) · cpupower-service.conf (inline `_chk_grep GOVERNOR`) · `_vss_udev` (all 3 rules, GPU_DPM_LEVEL-aware, EPP `cpu[0-9]*` matcher) · `_vss_modprobe` (mt7925e disable_aspm=1) · `_vss_nft` (**hard-fail on missing `echo-request` — the inbound-ping regression guard; IPv4-only**). **⊘ `_vss_ntsync_modules` REMOVED (v7.90.0) — the orchestrator's own `--description` string still says "Verify ntsync, …" but it no longer calls the sub (stale doc-string, FIX-doc Low/Low). NO `_vss_known_benign` (removed v7.77.0).**
- `_verify_static_user` — ENV_VARS (env.d, `_chk_grep` per var) + MangoHud. **CORRECTION:** MangoHud static verify is an **inline `_chk_file` + `_chk_grep "fps"`** existence probe (lines 2390–2393), NOT `_vmh_*` leaves. The `_vmh_existence_only` / `_vmh_order_checks` functions the prior body listed here are **`_ry_validate_mkinitcpio_hooks` sub-functions** (they validate the mkinitcpio HOOKS array's existence/ordering, called from the config-syntax validator — see §"config-format validators" below), and have nothing to do with MangoHud. The `_grep_mangohud_entry` format-validator (accepts bareword OR key=value OR `#`-comment) is invoked from `_rvc_dispatch` during config-syntax validation, not from `_verify_static_user`. **NO baloo (de-managed v7.77.0).**
- `_verify_static_packages` → leaves `_vsp_required` (**PKGS_ADD present — AND now the folded Vulkan check (vulkan-radeon + lib32-vulkan-radeon) with its own pacman-db-lock guard, v7.90.0**) · `_vsp_removed` (PKGS_DEL absent) · `_vsp_pacman_conf` (pacman.conf hooks/opts) · `_verify_static_services` · `_verify_static_syntax` · `_verify_static_checksum` → leaf `_vsc_check_one` per-file (embedded content SHA256 == installed file; **skips gracefully — returns 0 — when the generator yields `EXIT_GEN_NOUUID` and the root UUID is genuinely unresolved**, presence still verified separately).
- **Config-format validators (`_ry_validate_configs` → `_rvc_dispatch` per destination):** runs BEFORE deploy; dispatches each embedded body to a format checker (`_grep_kv` for loader/sdboot, `_grep_kparam` for cmdline, `_grep_sysctl_kv`, `_grep_modprobe_entry`, `_grep_regdomain_entry`, `_grep_udev_entry`, `_grep_nft_entry`, `_grep_envd_entry`, `_grep_cpupower_entry`, `_grep_mangohud_entry`, `_grep_ini_header` fallback; mkinitcpio.conf returns 0/skip). Separately, `_ry_validate_mkinitcpio_hooks` (with leaves **`_vmh_existence_only`** + **`_vmh_order_checks`**) and `_ry_validate_mkinitcpio_modules` validate the HOOKS/MODULES arrays. **This is the true home of `_vmh_*`.**

**NO known-benign INFO family** — the 5 `_kb_*` subs and their aggregator were removed (v7.77.0). Do not audit them.

**Runtime-kernel — `_verify_runtime_kparams`** (description string still says "…, clocksource" — stale, FIX-doc):
- Preemption INFO scaffold: caches `sudo -n dmesg` once, greps `Dynamic Preempt: <mode>` (INFO-only), then erases the cache. (This dmesg cache survives; only the TSC-demotion consumer `_vrk_clocksource` was removed.)
- `_vrk_cmdline` — **generic loop asserting EVERY `KERNEL_PARAMS` token + `rw` is present in `/proc/cmdline`** (so `amd_iommu=off` AND `ipv6.disable=1` AND `tsc=reliable` are all auto-asserted here); + preemption-model INFO.
- `_vrk_gpu_state` — `power_dpm_force_performance_level` == `$GPU_DPM_LEVEL` (auto) across `card*`.
- `_vrk_cpu_state` — scaling_driver=amd-pstate-epp, scaling_governor=$CPUPOWER_GOVERNOR, EPP=balance_performance, amd_pstate status, **`dynamic_epp=disabled`**, prefcore, boost=1.
- `_vrk_module_state` → subs `_vrkm_amdgpu` (hex-aware, splits param pairs once since values may contain `:`; no-ops without amdgpu.*) · `_vrkm_blacklist` (module_blacklist= scan); + usbcore.autosuspend, nvme_core ps_max_latency, zswap.enabled, nmi_watchdog, NVMe `[none]`. **⊘ `_vrkm_iommu` REMOVED (v7.90.0) — no more 0-iommu_groups hard-fail / dmesg AMD-Vi correlation. The `amd_iommu=off` directive is still asserted by `_vrk_cmdline` + `_vsb_cmdline`; only the sysfs-effect check is gone.**
- **⊘ `_vrk_clocksource` REMOVED (v7.90.0)** — the orchestrator no longer checks `clocksource=tsc` at sysfs, no longer HPET→fails, and no longer correlates dmesg TSC-demotion. The `tsc=reliable` directive is still asserted at the cmdline layer. The `_RY_DMESG_TSC` cache global is gone.

**Runtime-services — `_verify_runtime_services`:** `_vrsv_chk_active_enabled` · **`_vrsv_nft_assert_ping`** (live input chain accepts inbound IPv4 ping; warn-only — REPLACES the old `_vrsv_nft_assert_ndp`, which is gone) · `_vrsv_chk_nftables` (oneshot, no RemainAfterExit → judged by live ruleset policy-drop) · `_vrsv_chk_resolved` · `_vrsv_chk_cpupower_governor` · `_vrsv_sys_units` · `_vrsv_wifi_nm_backend` · `_vrsv_wifi_iwd_proc` · `_vrsv_wifi` · `_vrsv_masked_inactive`.

**Runtime-env — `_verify_runtime_env`** (description string still says "…, TCP, ZRAM, …" — stale, FIX-doc): `_vre_envvars` (`systemctl --user show-environment`) · `_vre_sysctl_runtime` (`/proc/sys`) · `_vre_fstab` (ext4 has `noatime,lazytime,commit=10`) · `_vre_ntsync` (state dispatch — **survives**) · `_vre_regdom` (`iw reg get`). **⊘ `_vre_tcp` (tcp_bbr) and `_vre_zram` (zram service + swap) BOTH REMOVED (v7.90.0)** — the runtime BBR check and the manages-nothing-but-asserts-zram check are gone. (BBR is still set via `SYSCTL_VALUES` `tcp_congestion_control=bbr` and asserted on-disk by `_vss_sysctl` + at `/proc/sys` by `_vre_sysctl_runtime`; only the dedicated `tcp_bbr`-module runtime probe was dropped.)

**Runtime-session — `_verify_runtime_session`:** `_vrs_nm_perms` (**system-connections 0600 root:root**) · `_vrs_vfat_skip` (skips perm check on vfat/undetermined boot) · `_vrs_installed_file_perms` (system 0644 / user 0600) · `_vrs_parent_dirs` → leaf `_vpd_dir_perm_check` (0755 system / 0700 user config dirs). **⊘ `_vrs_vulkan` REMOVED as a standalone leaf (v7.90.0) — the Vulkan-package check was folded into `_vsp_required` (static packages) instead.**

Actionable for §C:
- Confirm the **removed** effect-asserts (`_vrkm_iommu`, `_vrk_clocksource`, `_vre_tcp`, `_vre_zram`, `_vrs_vulkan`) leave no coverage gap that matters: `amd_iommu=off`/`tsc=reliable`/`bbr`/Vulkan-pkgs are all still asserted at the config and/or `/proc` layer — confirm the *directive*-level coverage is sufficient and the dropped *effect*-correlations were genuinely redundant advisory (the IOMMU 0-groups and HPET-fail checks were the only hard-fails among them; losing them converts two potential hard-fails into no-check — confirm that is the intended severity reduction and not a silent regression-guard loss).
...
- Confirm the three **stale orchestrator description strings** (`_verify_static_system` "ntsync", `_verify_runtime_kparams` "clocksource", `_verify_runtime_env` "TCP, ZRAM") are trimmed to match the current call bodies (FIX-doc, Low/Low) — the bodies are correct; only the `--description` text drifted.
- Confirm the `_vmh_*` correction: verify these are `_ry_validate_mkinitcpio_hooks` leaves and that MangoHud static verify is the inline `_chk_grep "fps"` existence probe — a recommendation that "changes a MangoHud directive" is gated by the format-validator (`_grep_mangohud_entry`, pre-deploy) + the checksum leaf (`_vsc_check_one`), NOT by `_vmh_*`.
...
3. **IOMMU live-effect assert (`_vrkm_iommu`) REMOVED (v7.90.0):** the 0-iommu_groups hard-fail under `amd_iommu=off` and the dmesg AMD-Vi correlation are gone — see "What changed since v7.88.3" item 3. The `amd_iommu=off` directive is still asserted (cmdline loop + `_vsb_cmdline`); only the sysfs-effect check was retired. `amd_iommu=off` does NOT break ROCm (`IOMMU Support: None`) — accepted trade-off. Assess whether losing the 0-groups hard-fail matters (it was one of only two removed hard-fails; the other was `_vrk_clocksource`'s HPET-fail).
4. **MangoHud `cpu_temp` RE-COMMENTED + `cpu_power` ADDED (v7.91.0 — inverted vs v7.88.3):** the shipped body now carries `# cpu_temp` (off) and a live `cpu_power`, with `gpu_temp` reordered before `gpu_core_clock`. Confirm **`cpu_power`** populates from the Zen 5 RAPL/`power1_average` hwmon as shipped (FIX only if it reads blank). MangoHud #1794 is now avoided by design (cpu_temp off, cpu_power on). The `cpu_temp` sensor-key question is dormant. This is the single most visible value-level change since v7.88.3.
...
13. **Removed surfaces:** confirm the audit does not re-flag baloo, `_kb_*`, `_ry_check_umip_disabled`, ICMPv6/NDP rules, or the firmware advisory (removed since v7.77.1), **NOR** the six subs removed in v7.90.0 (`_vre_tcp`, `_vre_zram`, `_vss_ntsync_modules`, `_vrkm_iommu`, `_vrk_clocksource`, `_vrs_vulkan`). None of these exist in v7.91.0 — do not verify them.
...
15. **(NEW v7.91.0) MangoHud `cpu_power` live + `cpu_temp` commented + GPU reorder:** the headline value-level delta — see item 4. Verify `cpu_power` populates; treat `cpu_temp` as dormant.
16. **(NEW v7.90.0) Effect-correlation asserts retired, directive asserts kept:** confirm `amd_iommu=off` (via `_vrk_cmdline` + `_vsb_cmdline`) and `tsc=reliable` (same two layers) remain asserted even though `_vrkm_iommu` (0-groups hard-fail) and `_vrk_clocksource` (HPET→fail + TSC-demotion) were removed. Assess whether losing those two *hard-fails* is an acceptable severity reduction (README AMD-Vi note was synced in v7.90.0).
17. **(NEW v7.90.0) Vulkan check folded into `_vsp_required`:** confirm the Vulkan-package assertion (vulkan-radeon + lib32-vulkan-radeon) now lives in static-packages with the ported pacman-db-lock guard, and that dropping the standalone `_vrs_vulkan` runtime-session leaf loses no coverage.
18. **(NEW) `_vmh_*` misattribution corrected:** confirm `_vmh_existence_only` / `_vmh_order_checks` are mkinitcpio-HOOKS validators, not MangoHud-verify leaves (prior body error).
19. **(NEW cosmetic) Stale orchestrator `--description` strings:** `_verify_static_system` ("ntsync"), `_verify_runtime_kparams` ("clocksource"), `_verify_runtime_env` ("TCP, ZRAM") name removed subs. FIX-doc, Low/Low — call bodies are correct.
20. **(NEW v7.89.0) Root guard defers to argparse:** invalid args exit 2 with the same usage message rooted or not; confirm no security-relevant change (the root-refusal for deploy modes still stands — only arg *validation* precedes it).
